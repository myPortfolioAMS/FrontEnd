...
function AlarmeManHe1(
  ...
  const [isGreaterThan, setGreaterThan] = useState (false);
...
//console.log("OldData:", OldData);
  ...
  const requestBody = {
    ...
    //P_60min_He1: P_60min_He1,
    //PrevisaoDeTroca: previsao_de_troca,
    //average_Consuption_bar: average_Consuption_bar,
...
// preciso trocar os 200 bar pela pressão máxima ( pressão inicial do cilindro - função ainda precisa ser escrita)
  CustoBar = (custo_cilindro_antigo / 200).toFixed(2);


  console.log('CustoBar:', CustoBar);
  console.log('custo_cilindro_antigo:', custo_cilindro_antigo);
  console.log('volume_antigo:', volume_antigo);
  ...
  custoHora =
    "R$" +
    (
      (custo_cilindro_antigo / 200) *
      (8+average_Consuption_bar)
    ).toFixed(2);
  ...
  console.log("PhojeHe1_1AM:", PhojeHe1_1AM);
  console.log("PhojeHe1_6AM:", PhojeHe1_6AM);
  ...
  Ux = Number((Date.now() / 1000).toFixed(0));
  ...
  date = new Intl.DateTimeFormat("pt-BR", {timeZone: 'UTC'}).format(currentTimestamp);
  console.log("date :", date); // get current date
  ...
  console.log("hour :", hour); // get current hour

    //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

  const requestBody_disparo_UpdateHe1 = {
    id: (Date.now() / 1000).toFixed(0),
    deviceID: "Aliine",
    Ux: Ux ,
    idGas: "He1",
    DataConectado: data_antigo,
    HoraConectado: hora_antigo,
    Pressao_He1: PatualHe1,
    PrevisaoDeTroca: previsao_de_troca,
    DataAlarme: date,
    HoraAlarme: hour,
 
    AlarmeHe1: String(AlarmeHe1),
    average_Consuption_bar: average_Consuption_bar,
    //AlarmeHe1: PRESSAO_ALARME_He1,
    Disparo_UpdateHe1_v1: "Disparo_UpdateHe1_v1",
  };

{/*  
  // função para se tar e gravar a pressão de alarme

  async function DisparoAlarme(requestBody_disparo, requestConfig) {
    // await api.post(registerUrl, requestBody, requestConfig)

    await api
      .post(
        "https://ag0kywbcyd.execute-api.sa-east-1.amazonaws.com/prod/alarmeevent?",
       // "https://z3rh89xnof.execute-api.sa-east-1.amazonaws.com/prod/alarme2?",
        requestConfig,
        requestBody_disparo
      )
      ...
      .then((response) => {
        console.log("Registro gravado com sucesso!");
        //alert("Cilindro de Hélio tem pressão próxima de 10% acima da Pressão de Alarme!");
      })
      .catch((error) => {
        console.log(error);
      });
...
*/}
  leak_test = leak * CustoBar;

  loss = Math.round(Number((leak/ (8+average_Consuption_bar))*100))
  LimiteSuperiorAlarmeHe = Number((AlarmeHe1*1.1).toFixed(0));
  ...
  useEffect(() => {
    ...
    if (Number(LimiteSuperiorAlarmeHe) > Number(PatualHe1)  ) {
      setGreaterThan(true);
    ...
    },[setGreaterThan, LimiteSuperiorAlarmeHe, PatualHe1]);
  */}
    console.log ("LimiteSuperiorAlarmeHe:", LimiteSuperiorAlarmeHe);
    console.log("type of LimiteSuperiorAlarmeHe:", typeof LimiteSuperiorAlarmeHe);
    console.log ("PatualHe1:", PatualHe1);
    console.log("type of PatualHe1:", typeof PatualHe1);
...
{/*
    ...
    console.log("Ux:", Ux); // timestamp atual
    console.log("Ux_later:", Ux_later); // timestamp atual + 1/2 hora

    if (Ux_later == "undefined" || Ux_later == null){
      console.log("Ux_later é undefined!")
    ...
    if (isGreaterThan === true) {
      ...
      DisparoAlarme(requestConfig, requestBody_disparo);
       
      console.log ("Disparo de alarme realizado com sucesso!")

      Ux_later = Ux + 1800;

      }  else {

        console.log ("Não houve disparo de alarme!")
...
<S.FeaturedContainer>
            ...
            <S.FeaturedMoneyContainer> 
                 ...
                 <span className= "featuredMoney">{String(PatualHe1)}
            ...
            <S.FeaturedMoneyContainer> 
                ...
                <span className="featuredMoneyRate"> <ArrowDownwardOutlinedIcon className="featuredIconManometer"/> {valorTroca} </span> 
